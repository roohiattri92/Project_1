...
public class autosugestion_program {
	...
	//	god.findElement(By.xpath("(//div[@class='s-suggestion-container']/div)[4]")).click();
		List<WebElement> h=	god.findElements(By.xpath("//div[@class='s-suggestion-container']"));//if we use findelements then we cannot click easily need to write it dwown with size
		h.size();
		System.out.println(h.size());
		h.get(h.size()-6).click();
